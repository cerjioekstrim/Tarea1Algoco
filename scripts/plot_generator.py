...
#
# Lee data/measurements/matrix_measurements.csv (generado por
# matrix_multiplication.cpp), promedia las 3 muestras (a, b, c) de cada
# combinacion (algoritmo, n, tipo, dominio) y genera graficos de tiempo y
# memoria vs n en data/plots/.
...
MEASUREMENTS_FILE = os.path.join("..", "data", "measurements", "matrix_measurements.csv")
...
def graficar_metrica(df, tipo, dominio, columna, ylabel, titulo, nombre_archivo):
    ...
    plt.xscale("log")
    plt.yscale("log")
    plt.xlabel("n (dimension de la matriz n x n)")
    plt.ylabel(ylabel)
    plt.title(titulo)
    plt.legend()
